...
Kakao Local API는 일반 버스정류소 데이터셋을 제공하지 않으므로 `TRANSPORT`는 Kakao 지하철 `SW8`에 선택적 TAGO 결과를 합친다. `BUS_STOP_PROVIDER=tago`이면 TAGO의 `getCrdntPrxmtSttnList`로 조회 중심 500m 안의 정류소 최대 100개를 요청하고 반환 좌표로 거리를 다시 계산해 500m 안만 합친다. 공공데이터포털에서 해당 API를 활용 신청한 뒤 발급된 일반 인증키(Decoding)를 서버에만 저장한다.

주변 조회의 `counts`는 공급자의 전체 검색 건수가 아니라 실제로 정규화해 반환한 장소 수다. 프론트는 이 실제 장소 좌표를 사용하며, 지도 축소 단계에서는 가까운 장소를 좌표 중심 군집으로 묶고 확대 단계 3 이하에서는 개별 핀으로 표시한다. 모든 카테고리의 캐시 미적중 시 Kakao 호출은 최대 15회이고 TAGO를 켰으면 정류소 호출 1회를 더한다. 동일한 정규화 좌표·반경·카테고리는 합친 결과를 10분 캐시한다.
