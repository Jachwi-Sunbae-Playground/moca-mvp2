...
이 저장소의 주 작업 도구는 Codex다. 기존 Claude Code 설정도 남겨 어느 도구로 수정하더라도 같은 최소 규칙과 검사가 적용되도록 한다.
...
한쪽만 고치면 [정합성 검사](documentation.md)가 실패한다. 중복 설정은 제거하지 않고 기계 검사로 어긋남을 막는다.
...
두 도구가 같은 `SKILL.md` 형식을 사용하므로 현재 검토 절차는 미러로 유지한다.
